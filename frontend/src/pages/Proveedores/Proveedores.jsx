...
const Proveedores = () => {
  ...
  useEffect(() => {
    if (usuario?.rol === 'admin' || usuario?.rol === 'demo_admin') fetchSuppliers();
  ...
  if (usuario && usuario.rol !== 'admin' && usuario.rol !== 'demo_admin') {
    return (
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await actualizarProveedor(editing._id, form);
      } else {
      ...
      alert({ icon: 'error', title: 'Error', message: obtenerMensajeErrorApi(err, 'Error al guardar proveedor') });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      alert({ icon: 'error', title: 'Error', message: obtenerMensajeErrorApi(err, 'Error al eliminar proveedor') });
    }
  };
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {suppliers.length === 0 ? (
                ...
                suppliers.map((sup) => (
                  <tr key={sup._id} className="border-t border-ios-separator/30 hover:bg-ios-hover/[0.03] transition-colors">
